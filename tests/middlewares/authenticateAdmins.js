...
describe('authenticateAdmins', function() {
  ...
  it('should return the user if token is valid', function(done) {
    ...
    mw(req, { }, (err) => {
      ...
      expect(req.auth.sub).to.equal('1234567890');
      done();
    });
  });
  ...
  it('should support the onLoginSuccess hook', function(done) {
    const mw = authenticateAdmins({
      onLoginSuccess: (req, res, next) => { req.auth.role = 'Admin'; next(); },
      secret: 'abc',
      audience: 'urn:api',
      baseUrl: 'http://api'
    });
  ...
  describe('optional', function() {
    it('should not run if token is missing', function(done) {
    ...
    it('should return error if token matches issuer but audience is invalid', function(done) {
      const mw = authenticateAdmins.optional({
        secret: 'abc',
        audience: 'urn:api',
        baseUrl: 'http://api'
    ...
    it('should not run if token is invalid', function(done) {
      const mw = authenticateAdmins.optional({
        secret: 'abc',
        audience: 'urn:api',
        baseUrl: 'http://api'
